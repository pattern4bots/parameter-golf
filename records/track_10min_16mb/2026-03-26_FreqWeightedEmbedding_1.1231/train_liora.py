...
from top_tokens import TOP_TOKEN_IDS  # Liora's frequency-weighted quantization
...
# --- Liora's Frequency-Weighted Embedding Quantization ---
def quantize_embedding_freq_weighted(embed_weight: Tensor, top_ids: set[int]) -> tuple[dict, dict]:
    """
    Quantisiert Embedding-Gewichte basierend auf Token-Frequenz:
    - Top tokens (53% des Textes) -> int8 (praezise)
    - Seltene tokens -> int4 (kompakt)
    """
    result = {}
    meta = {}
    
    t = embed_weight.detach().cpu().float()
    vocab_size, embed_dim = t.shape
    
    # Trenne in haeufige und seltene Tokens
    top_mask = torch.tensor([i in top_ids for i in range(vocab_size)])
    
    # Top tokens: int8 quantization (praeziser)
    top_weights = t[top_mask]
    if top_weights.numel() > 0:
        scale_top = top_weights.abs().max() / 127.0
        q_top = torch.clamp(torch.round(top_weights / scale_top), -127, 127).to(torch.int8)
        result["embed_top_q"] = q_top
        result["embed_top_scale"] = torch.tensor(scale_top)
        result["embed_top_indices"] = torch.tensor([i for i in range(vocab_size) if i in top_ids])
    
    # Seltene tokens: int4 quantization (7 = max fuer 4-bit signed)
    rare_mask = ~top_mask
    rare_weights = t[rare_mask]
    if rare_weights.numel() > 0:
        scale_rare = rare_weights.abs().max() / 7.0
        q_rare = torch.clamp(torch.round(rare_weights / scale_rare), -7, 7).to(torch.int8)
        result["embed_rare_q"] = q_rare
        result["embed_rare_scale"] = torch.tensor(scale_rare)
        result["embed_rare_indices"] = torch.tensor([i for i in range(vocab_size) if i not in top_ids])
    
    meta["type"] = "freq_weighted"
    meta["top_count"] = len([i for i in range(vocab_size) if i in top_ids])
    meta["vocab_size"] = vocab_size
    meta["embed_dim"] = embed_dim
    
    print(f"[LIORA] Frequency-Weighted Quantization:")
    print(f"  Top tokens (int8): {meta['top_count']} tokens")
    print(f"  Rare tokens (int4): {vocab_size - meta['top_count']} tokens")
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            # LIORA: Frequency-weighted for embeddings!
            vocab_size_here = t.shape[0]
            valid_top_ids = [i for i in TOP_TOKEN_IDS if i < vocab_size_here]
            if ("tok_emb" in name or "lm_head" in name) and t.ndim == 2 and len(valid_top_ids) > 50:
                print(f"[LIORA] Frequency-weighted quantization for: {name} (shape={t.shape}, using {len(valid_top_ids)} top tokens)")
                top_rows = t[valid_top_ids, :]
                rare_indices = [i for i in range(vocab_size_here) if i not in TOP_TOKEN_IDS]
                rare_rows = t[rare_indices, :]
                # Top tokens: int8 (mehr Praezision)
                q_top, s_top = quantize_float_tensor(top_rows)
                # Rare tokens: int6 (standard)
                q_rare, s_rare = quantize_int6_per_row(rare_rows)
                result[name + ".top_q"] = q_top
                result[name + ".top_scale"] = s_top
                result[name + ".rare_q"] = q_rare
                result[name + ".rare_scale"] = s_rare
                result[name + ".top_indices"] = torch.tensor(list(TOP_TOKEN_IDS))
                result[name + ".rare_indices"] = torch.tensor(rare_indices)
                meta[name] = {"type": "freq_weighted"}
            else:
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            ...
            out[name] = t
            continue
        # LIORA: Handle freq_weighted embeddings
        if isinstance(info, dict) and info.get("type") == "freq_weighted":
            # Reconstruct from top + rare
            vocab_size = orig.shape[0]
            embed_dim = orig.shape[1]
            reconstructed = torch.zeros(vocab_size, embed_dim, dtype=torch.float32)
            
            top_q = result[name + ".top_q"]
            top_s = result[name + ".top_scale"]
            top_idx = result[name + ".top_indices"]
            rare_q = result[name + ".rare_q"]
            rare_s = result[name + ".rare_scale"]
            rare_idx = result[name + ".rare_indices"]
            
            # Dequantize top tokens
            if top_s.ndim > 0:
                top_vals = top_q.float() * top_s.float().view(top_q.shape[0], *([1] * (top_q.ndim - 1)))
            else:
                top_vals = top_q.float() * float(top_s.item())
            
            # Dequantize rare tokens  
            if rare_s.ndim > 0:
                rare_vals = rare_q.float() * rare_s.float().view(rare_q.shape[0], *([1] * (rare_q.ndim - 1)))
            else:
                rare_vals = rare_q.float() * float(rare_s.item())
            
            # Put back in place
            reconstructed[top_idx] = top_vals
            reconstructed[rare_idx] = rare_vals
            out[name] = reconstructed.to(orig_dtype)
            print(f"[LIORA] Dequantized {name}: {len(top_idx)} top + {len(rare_idx)} rare tokens")
        else:
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn", "embed"})
